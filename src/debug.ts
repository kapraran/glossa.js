...
export const debugTokenizer = function(
  ...
  for (let token of lexerResult.tokens) {
    ...
    if (toHTML) {
      ...
      currentOffset = (token.endOffset || token.startOffset + token.image.length) + 1;
      lines = token.endLine || lines
    }
  }

  if (toHTML) {
      const htmlTemplate = readFileSync(resolve(__dirname, '../assets/tokenizer.html'), 'utf8')
      const editedHtml = htmlTemplate
        .replace('$HTML_CONTENTS', htmlContents)
        .replace('$LINES', lines.toString())

      writeFileSync(resolve(__dirname, '../dist/out.html'), editedHtml, 'utf8')
  }
};
